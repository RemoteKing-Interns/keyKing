...
// Add type for the Variant document
interface IVariantDocument extends mongoose.Document {
  rkid: string;
  name: string;
  ...
  tools: Array<{ name: string; models: string[] }>;
  programmingInfo: {
    method: string;
    steps: string[];
  };
...
const seedDatabase = async () => {
  try {
    ...
    await mongoose.connection.db.dropCollection('variants').catch(() => {
      console.log('Variants collection does not exist, creating a new one...');
    });
